...
def extract_all(client: openai.OpenAI, model: str, pages: list[Page]) -> ExtractionResult:
    ...
    fees: FeeScheduleExtraction = _parse(
        ...
        (
            ...
            "with the row label sometimes sitting BETWEEN a price line and the rest of its own "
            "descriptor. For raw_text, copy ONLY the shortest verbatim string that contains the "
            "amount/rate and sits on a SINGLE line of the text (e.g. '$55,000', '$0.12 PMPM', "
            "'$0.03 per claim', '$235 per programming hour', 'Included', 'Quoted upon request'). Do NOT "
            "append words from a different line into raw_text; put any remaining descriptor context in "
            "`qualifier`.\n"
            "ATTRIBUTION (critical): each service's cost belongs to THAT service. Never move a price "
            "that belongs to one service into a different service's row or qualifier. When a price line "
            "appears directly ABOVE a service label and that service's descriptor continues on the line "
            "BELOW the label, the price belongs to the LABELED service (the one in the middle), not the "
            "service above it. After extracting, sanity-check: a service whose own Cost cell shows an "
            "amount or rate must carry that amount, not 'Included'.\n"
            "value_kind: 'numeric' for any dollar/PMPM/per-hour/etc amount (set value + unit); "
            "'included' ONLY when that service's own Cost literally reads 'Included' / 'No charge' (not "
            "because a nearby line said Included); 'quoted_on_request' for 'Quoted upon request'; "
            "'pass_through' for a pure pass-through cost; 'conditional' for a tiered/contingent charge "
            "(e.g. an hourly fee that only applies on a late change). A per-hour or contingent fee is "
            "still priced -> use 'numeric' or 'conditional' with the amount, NEVER 'included'. For "
            "non-numeric kinds leave value and unit null.\n"
            "When one service has MULTIPLE priced lines, emit one row per line with `variant` "
            "(e.g. Appeals -> 'DMR appeal'/'administrative appeal'/'clinical appeal'/'external appeal'). "
            "When a single Cost cell offers a free standard option AND a priced option, emit BOTH as "
            "variants (e.g. 'standard ... included; custom ... $X' -> one variant value_kind='included' "
            "and one variant with the $X amount). When a service includes a base allotment and charges "
            "for overage, emit the included-base row AND the overage row under the SAME service_name "
            "(e.g. portal access -> variant='included allotment' value_kind='included' qualifier='4 "
            "users included', and variant='additional user' with its per-user amount). Do NOT create a "
            "bare 'N users' service with no service it belongs to.\n"
